package com.qhdp.delay.consumer;
...
public class ConsumerDelayedVoucherReminder implements ConsumerTask {
    ...
    public void execute(final String content) {
        try {
            ...
            SeckillVoucherFullVO voucherFull = seckillVoucherService.queryByVoucherId(voucherId);
            if (voucherFull == null) {
    ...
    private Set<String> buildAudienceUserIds(SeckillVoucherFullVO voucherFull) {
    ...
    private List<Long> readUserIdsFromLevelSets(List<Integer> levels, int count) {
        ...
        List<String> keys = new ArrayList<>(levels.size());
        ...
        if (keys.size() == 1) {
            Set<Long> r = redisUtils.sDistinctRandomMembers(keys.get(0), Math.max(count, 1), Long.class);
        ...
        String dest = RedisKeyBuild.createRedisKey(RedisKeyManage.SECKILL_USER_LEVEL_MEMBERS_UNION_TAG_KEY, label);
        String base = keys.get(0);
        Collection<String> others = keys.subList(1, keys.size());
        try {
            redisUtils.sUnionAndStore(base, others, dest);
            redisUtils.expire(dest, 60);
        } catch (Exception e) {
            log.warn("[DELAY_REMINDER_CONSUMER] SET并集失败 levels={} label={}", levels, label, e);
        }
        Set<Long> r = redisUtils.sDistinctRandomMembers(dest, Math.max(count, 1), Long.class);
        return new ArrayList<>(r);
    }
    ...
    private List<Long> readTopBuyersFromRedis(Long shopId, int count, int days) {
        try {
            ...
            if (dailyKeys.size() == 1) {
                Set<Long> topSet = redisUtils.zReverseRange(
            ...
            String destKey = RedisKeyBuild.createRedisKey(
                    RedisKeyManage.SECKILL_SHOP_TOP_BUYERS_UNION_TAG_KEY,
                    shopId,
                    rangeLabel
            );
            String base = dailyKeys.get(0);
            Collection<String> others = dailyKeys.subList(1, dailyKeys.size());
            try {
                redisUtils.zUnionAndStore(base, others, destKey);
                redisUtils.expire(destKey, 60);
            } catch (Exception e) {
                log.warn("[DELAY_REMINDER_CONSUMER] ZSET并集失败 shopId={} range={}", shopId, rangeLabel, e);
            }
            Set<Long> topSet = redisUtils.zReverseRange(destKey, 0, Math.max(count - 1, 0), Long.class);
            return new ArrayList<>(topSet);
        } catch (Exception ex) {
    ...
    private int notifyUsers(Long voucherId, Date beginTime, Set<String> userIds) {
        int notifyCount = 0;
        for (String userIdStr : userIds) {
            if (StrUtil.isBlank(userIdStr)) { continue; }
            boolean shouldNotify;
            try {
                shouldNotify = redisUtils.setIfAbsent(
